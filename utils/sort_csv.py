...
def create_p_np_annotations_rel(folder_path): 
    """
    Creates csv annotations for dataset
    ...
    for r_dir, sub_dir_list, file_list in os.walk(folder_path):
        ...
        with open(os.path.join(r_dir, 'cleaned_dataset.csv'), 'w') as csvfile:
            ...
            for file_path in file_list:
                if 'np' in file_path:
                    writer.writerow({'file_name': os.path.join('LSTM/data/violin/raw', file_path), 'class': 'np'})
                else: 
                    writer.writerow({'file_name': os.path.join('LSTM/data/violin/raw', file_path), 'class': 'p'})

if __name__ == '__main__':
    create_p_np_annotations(folder_path='LSTM/data/violin')